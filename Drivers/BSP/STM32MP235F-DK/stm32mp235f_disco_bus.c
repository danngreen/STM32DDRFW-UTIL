...
int32_t BSP_I2C_Init(void)
{
  ...
  hbus_i2c.Instance = BUS_I2C;

  if (I2cInitCounter == 0U)
  {
    I2cInitCounter++;
    ...
    if (HAL_I2C_GetState(&hbus_i2c) == HAL_I2C_STATE_RESET)
    {
      ...
      {
#endif /* USE_HAL_I2C_REGISTER_CALLBACKS */
      if (MX_I2C_Init(&hbus_i2c,
                       I2C_GetTiming(HAL_RCC_GetFreq(RCC_CLOCKTYPE_ICN_HS_MCU), BUS_I2C_FREQUENCY)) != HAL_OK)
      {
...
static void I2C_MspDeInit(I2C_HandleTypeDef *hI2c)
{
  ...
  /* Disable I2C clock */
  BUS_I2C_CLK_DISABLE();
  (void)ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOG, BUS_I2C_SCL_PIN_RIF_RES);
  (void)ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOI, BUS_I2C_SDA_PIN_RIF_RES);
...
static int32_t I2C_Recv(uint16_t DevAddr, uint8_t *pData, uint16_t Length)
{
  if (HAL_I2C_Master_Receive(&hbus_i2c, DevAddr, pData, Length, 10000) == HAL_OK)
  {
    return BSP_ERROR_NONE;
  }
...
static int32_t I2C_Send(uint16_t DevAddr, uint8_t *pData, uint16_t Length)
{
  if (HAL_I2C_Master_Transmit(&hbus_i2c, DevAddr, pData, Length, 10000) == HAL_OK)
  {
    return BSP_ERROR_NONE;
  }
